...
class ServerThread extends Thread
{
	...
	public void run() {		
		...
		int hm = line.hashCode();//明文的哈希值
			System.out.println("该明文的哈希值为："+hm);
 
			/*int hm = message.hashCode();//明文的哈希值
			System.out.println("该明文的哈希值为："+hm);*/
 
			ServerThread.getRandomP(z);
			x = ServerThread.getRandoma(p);
			y = ServerThread.calculatey(x, g, p);
			System.out.println("计算机随机生成的素数P:" + p);
			System.out.println("求得其生成元g:" + g);
			System.out.println("私钥<x,g,p>为: (" + x + "," + g + "," + p + ")");
			System.out.println("公钥<y,g,p>为:" + "(" + y + "," + g + "," + p
					+ ")");
			
			ServerThread.encrypt(hm, x, p, g);
			ServerThread.decrypt(C, C1, hm,g, p,y);
